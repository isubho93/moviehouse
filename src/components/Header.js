...
export const Header = () => {
  ...
  const [lightMode, setLightMode] = useState(JSON.parse(localStorage.getItem("Light Mode")) || false);
  ...
  return (
    <header>
      <nav className="bg-white border-b-2 border-gray-200 border-gray-200 dark:bg-gray-900 dark:border-b1 dark:border-gray-900">

        <div className="container flex flex-wrap items-center justify-between mx-auto p-2  ">

          <Link to="/" className="ml-3 flex space-x-4 flex items-center space-x-3 rtl:space-x-reverse">
            <img src={Logo} className=" h-6  sm:h-9" alt="Movie House Logo" />
            <span className="self-left text-2xl font-semibold whitespace-nowrap text-gray-900 dark:text-white">Movie House</span>
          </Link>

          <div id="mobile-nav" className="flex md:order-2 mr-2">
